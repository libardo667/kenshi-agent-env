...
""".lstrip(),
    ...
    stats.parent.mkdir()
    stats.write_text("{}\n", encoding="utf-8")
    generated_source = workspace / "mutants" / batch.source_path
...
def test_input_digest_ignores_generated_python_and_test_caches(
    tmp_path: Path,
) -> None:
    repo = tmp_path / "repo"
    ...
    baseline = campaign._input_digest(repo, {"src", "tests"})

    bytecode = repo / "src" / "nested" / "__pycache__" / "generated"
    bytecode.parent.mkdir(parents=True)
    bytecode.write_bytes(b"generated bytecode")
    pytest_cache = tests / "unit" / ".pytest_cache" / "nodeids"
    pytest_cache.parent.mkdir(parents=True)
    pytest_cache.write_text('["test_module.py::test_value"]\n', encoding="utf-8")

    assert campaign._input_digest(repo, {"src", "tests"}) == baseline

    source.write_text("VALUE = 2\n", encoding="utf-8")
    assert campaign._input_digest(repo, {"src", "tests"}) != baseline


def test_completed_batch_refuses_inputs_that_changed_while_it_ran(
    tmp_path: Path,
) -> None:
    repo = tmp_path / "repo"
    (repo / "src" / "kenshi_agent").mkdir(parents=True)
...
""".lstrip(),
    ...
    campaign._assert_batch_inputs_unchanged(repo, workspace)
    test.write_text("def test_recall(): assert False\n", encoding="utf-8")

    with pytest.raises(campaign.MutationCampaignStateError) as exc_info:
        campaign._assert_batch_inputs_unchanged(repo, workspace)
...
@pytest.mark.parametrize("fingerprint_content", [None, "{"])
def test_completed_batch_refuses_a_missing_or_malformed_input_fingerprint(
    fingerprint_content: str | None,
    tmp_path: Path,
) -> None:
    repo = tmp_path / "repo"
    (repo / "src" / "kenshi_agent").mkdir(parents=True)
    ...
    if fingerprint_content is None:
        ...
        fingerprint.write_text(fingerprint_content, encoding="utf-8")
...
def test_batch_workspace_lock_refuses_a_concurrent_owner_and_releases(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "memory"

    with campaign._batch_workspace_lock(workspace):
        with pytest.raises(RuntimeError, match="already in use"):
            with campaign._batch_workspace_lock(workspace):
                pytest.fail("a second process boundary entered the same workspace")

    with campaign._batch_workspace_lock(workspace):
        assert (workspace / ".campaign.lock").is_file()
...
""".lstrip(),
    ...
    generated_metadata = Path(f"{generated_source}.meta")
    generated_metadata.write_text(
        '{"exit_code_by_key": {"kenshi_agent.memory.x_recall__mutmut_1": 1}}\n',
        encoding="utf-8",
    )
    ...
    assert generated_source.read_text(encoding="utf-8") == "instrumented\n"


@pytest.mark.parametrize(
    ("present", "source_content", "metadata_content", "expected"),
    [
        (frozenset(), b"", "", True),
        (frozenset({"stats"}), b"", "", False),
        (frozenset({"stats", "source"}), b"x", "", False),
        (
            frozenset({"stats", "source", "metadata"}),
            b"",
...
def test_batch_cache_is_complete_only_for_a_finished_mutant_generation(
    ...
    generated_source = workspace / "mutants" / batch.source_path
    stats = workspace / "mutants" / "mutmut-stats.json"
    metadata = Path(f"{generated_source}.meta")
    if present:
        generated_source.parent.mkdir(parents=True)
    if "stats" in present:
        stats.write_text("{}\n", encoding="utf-8")
    if "source" in present:
        generated_source.write_bytes(source_content)
    if "metadata" in present:
        metadata.write_text(metadata_content, encoding="utf-8")

    assert campaign._batch_cache_is_complete(workspace, batch) is expected


def test_workspace_invalidates_interrupted_mutant_generation_with_identical_inputs(
    tmp_path: Path,
) -> None:
    repo = tmp_path / "repo"
    (repo / "src" / "kenshi_agent").mkdir(parents=True)
    (repo / "tests").mkdir()
    (repo / "src" / "kenshi_agent" / "runtime.py").write_text(
...
""".lstrip(),
    ...
    batch = MutationBatch(
        name="runtime",
        source_path="src/kenshi_agent/runtime.py",
        mutant_pattern="kenshi_agent.runtime.*",
    )
    workspace = prepare_batch_workspace(repo, batch)
    stats = workspace / "mutants" / "mutmut-stats.json"
    stats.parent.mkdir()
    ...
    generated_source = workspace / "mutants" / batch.source_path
    generated_source.parent.mkdir(parents=True)
    generated_source.write_text("", encoding="utf-8")
    generated_metadata = Path(f"{generated_source}.meta")
    generated_metadata.write_text(
        '{"exit_code_by_key": {}}\n',
        encoding="utf-8",
...
def test_run_artifact_preserves_exact_batch_evidence(
    ...
    assert collision_artifact == (
        tmp_path / "runs" / "mutation" / "20260727T214512Z-env-live-1.json"
    )
    assert second_collision_artifact == (
        tmp_path / "runs" / "mutation" / "20260727T214512Z-env-live-2.json"
    )
    expected_payload = {
        "batch": "env.live",
    ...
    assert json.loads(artifact.read_text(encoding="utf-8")) == expected_payload
    assert json.loads(collision_artifact.read_text(encoding="utf-8")) == expected_payload
    assert (
...
def test_main_gates_actionable_results_for_run_and_results_commands(
    ...
    def fake_lock(workspace: Path) -> nullcontext[None]:
        dependency_calls.append(("lock", workspace))
        return nullcontext()

    monkeypatch.setattr(campaign, "_batch_workspace_lock", fake_lock)
    ...
    monkeypatch.setattr(
        campaign,
        "_assert_batch_inputs_unchanged",
        lambda repo_root, workspace: dependency_calls.append(
            ("verify", repo_root, workspace)
        ),
    )
    summary = MutationSummary(
        counts={"survived": 1},
    ...
    assert dependency_calls == [
        ...
        ("lock", tmp_path / ".mutation-workspaces" / "memory"),
        ("prepare", tmp_path, batch),
        *(
            [("verify", tmp_path, tmp_path / "workspace")]
            if command == "run"
            else []
        ),
...
def test_main_reports_workspace_state_refusal_on_stderr(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    class RefusingLock:
        def __enter__(self) -> None:
            raise campaign.MutationCampaignStateError("workspace busy")
    ...
    monkeypatch.setattr(
        campaign,
        "_batch_workspace_lock",
        lambda workspace: RefusingLock(),
    )

    assert campaign.main(["results", "memory"]) == 2
    captured = capsys.readouterr()
    assert captured.out == ""
    assert captured.err == "mutation campaign refused: workspace busy\n"
...
def test_main_accepts_one_mutation_worker(
    ...
    monkeypatch.setattr(
        campaign,
        "_assert_batch_inputs_unchanged",
        lambda repo_root, workspace: None,
    )
    monkeypatch.setattr(
        campaign,
        "_read_batch_results",
