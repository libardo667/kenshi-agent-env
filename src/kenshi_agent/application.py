"""Public application composition root used by every operator entrypoint."""
...
from .scenario_validation import ScenarioFixtureError, validate_attested_snapshot
from .session_log import SessionLogger
from .speech import SpeechUnavailableError, default_narrator
from .telemetry import TelemetryReader


class ScenarioProofLoader(Protocol):
    def __call__(self, path: Path) -> ScenarioAttestation: ...
...
def _apply_run_overrides(
    ...
    *,
    ...
    if attestation_path is not None:
        if scenario_proof_loader is None:
            raise SystemExit(
                "Scenario-attested runs require the public tooling proof loader."
            )
        try:
            scenario_attestation = scenario_proof_loader(
...
async def _run_command(
    ...
    *,
    ...
    config = _apply_run_overrides(
        ...
        args,
        scenario_proof_loader=scenario_proof_loader,
    )
    _validate_run_platform(config, args)
    _validate_attested_live_scenario(config)
...
def main(
    ...
    *,
    ...
    if args.subcommand == "run":
        return asyncio.run(
            _run_command(args, scenario_proof_loader=scenario_proof_loader)
        )
