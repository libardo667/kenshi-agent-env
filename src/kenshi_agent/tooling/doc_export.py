...
from ..config import PLANNER_OUTPUT_POLICY
from ..planners.base import planner_output_policy_language
...
def _planner_output_contract() -> str:
    """Render the hosted contract from the same owner as schema and compiler."""

    policy = PLANNER_OUTPUT_POLICY
    steps = PlanProposal.model_json_schema()["properties"]["steps"]
    lines = [
        GENERATED_MARKER,
        "",
        ...
        "",
        "```text",
        f"policy                    {policy.cardinality_phrase}",
        f"schema minItems           {steps['minItems']}",
        f"schema maxItems           {steps['maxItems']}",
    ...
    lines.extend(planner_output_policy_language(policy).splitlines())
    lines.append("")
    return "\n".join(lines)
...
def export_docs(output_dir: Path) -> list[Path]:
    ...
    documents = {
        ...
        "RESEARCH_EVIDENCE_INDEX.md": render_research_index(),
