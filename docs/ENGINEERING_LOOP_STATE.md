...
`environment.dispatch()`, but `LiveEnvironment` then waits an unbounded polite
interval for a quiet input turn. Selection, UI phase, pause state, capabilities,
control mode, or human control can all change during that wait. Two narrow
in-lease fences existed — semantic startup label/bounds re-reads and the exact
client-size recheck — but ordinary gameplay dispatch carried no step-specific
typed authority to the actual issue point.

Scope:

- Add `ExecutionToken`: a bounded authorization built at executor validation
  time carrying plan/step/command identity, control mode, validated revision,
  the plan's assumptions, the step's preconditions, and a deferred accessor to
  the world-state store.
- Carry it through `AgentEnvironment.dispatch()` and the stateful movement
  option. Environments without a real lease accept it without re-checking.
- Run the fence inside the acquired lease, after calibration and immediately
  before the first primitive.
- Reuse `evaluate_conditions` so the boundary keeps five-valued
  capability-aware semantics rather than a parallel boolean path.
- Emit zero primitives on rejection and return an explicit typed receipt.
- Record the decision on the receipt, in lifecycle events, and in metrics.

Non-goals:

- No shortened lease timeout and no weakened polite human handoff.
- No versioned calibration identity beyond the existing client-size gate; the
  token is the intended carrier when P4 lands.
- No change to native-assisted issue-time DLL fences, command IDs, or
  acknowledgement semantics.
- No single-step token, which has no plan assumptions or step preconditions.
- No live Kenshi execution.

Acceptance criteria:

- A deterministic test blocks inside a fake input lease, publishes conflicting
  state, releases the lease, and proves zero primitives were emitted.
- An unchanged acceptable state executes exactly once.
- Capability withdrawal, selection change, unpause, control-mode change,
  revision regression, missing canonical observation, human input, and
  emergency stop each independently prevent dispatch.
- A proven non-dispatch releases its reservation.
- Receipts and metrics distinguish post-lease rejection from pre-lease policy
  rejection and from issued/completed/inconclusive outcomes.
- The calibration brake still fails closed before the fence.

Result: complete and offline-verified. `src/kenshi_agent/input_boundary.py`
owns the token and decision; `env/live.py` runs it inside the lease;
`continuous_executor.py` builds it and emits `input_boundary_revalidated` /
`input_boundary_rejected`; `ActionReceipt.input_boundary` carries the typed
report. Evidence is 251 passing tests (16 new), Ruff, mypy across 51 source
files, compileall, regenerated byte-exact schema parity, doctor, seeds 7/11/19
at 25/13/13 actions, and the portable continuous proof unchanged at one
completed plan and two later-revision steps. This is automated portable
evidence only; no live Kenshi run has exercised the fence against a real input
lease. See `docs/ADR_INPUT_BOUNDARY_AUTHORITY.md`.

## Previous slice: P0 semantic launch and explicit control ownership
...
- `.venv/bin/python -m pytest -o addopts='' -q`: 251 passed, up from 235.
- `.venv/bin/ruff check .`: passed.
- `.venv/bin/mypy src`: passed, 51 source files.
...
- `schemas/receipt.schema.json` was regenerated for the new nullable
  `input_boundary` report; a fresh export then matched `schemas/`
  byte-for-byte.
- Fourteen focused live-environment tests block inside a fake lease and prove
  zero primitives on selection change, capability withdrawal, unpause, human
  input, emergency stop, control-mode change, revision regression, and a
  missing canonical observation. An unchanged state still executes exactly
  once, and a tokenless dispatch keeps the legacy path with no report.
- The client-size calibration brake still raises before the fence runs, and the
  token records no decision in that case.
- Two executor tests prove the token reaches dispatch with matching
  plan/step/command identity and validated revisions `[1, 2]`, and that a
  post-lease rejection releases its reservation, logs
  `input_boundary_rejected` before `plan_budget_released`, and reports
  `input_boundary_rejections: 1` with `budget_releases: 1` and
  `plan_steps_succeeded: 0`.
- Mock seeds 7, 11, and 19 retained the accepted one-day results in 25, 13, and
  13 actions.
- Continuous run `p3-boundary-continuous-proof` retained one completed plan,
  two later-revision steps, two committed reservations, zero rejected actions,
  and zero boundary events, because the mock environment has no input lease.
